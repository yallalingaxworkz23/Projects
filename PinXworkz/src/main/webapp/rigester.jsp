...
<nav class="navbar navbar-expand-lg bg-body-tertiary"  >
    ...
    <div class="container">
        ...
        <form action="vmanagemnet" method="post" onclick="  form()">

           
            <label for="name">Company Name:<span id="validname" style="margin-left: 20px"></span></label> <div class="input">
            <input type="text" id="name" name="name" placeholder="Enter your company" onchange="namevalidation()" required="required">
             </div>
...
<!--             <Span id="validcontajax" style="color:red;" ></Span> -->
            ...
            <input type="email" id="emailid" name="emailid" placeholder="Enter your alternet email Id " onchange="emailvalidation()" required="required">
               ...
               <div class="input1">
                 <label for="wibsite">Web Site:<Span id="websitevalid" style="margin-left:20px;"></Span></label><span id="ok" ></span>
            ...
            <p style="margin-left: 660px; margin-top: -30px;">Already have Account?  <a href="login.jsp">Sign in</a></p>
        </form>
    </div>
    
    
     <div class="footer" style="margin-top: 70px;background-color: #88ceec;">
      <div>
        ...
        <p style="margin-left:750px;">follow us on</p> 
...
<script>
    function namevalidation(){
        ...
        if(names.length!="" && names.length>4 && names.length<=25){
    ...
    function locationvalidation() {
    	...
    	var button=document.getElementById("button");
    	if(locations!="" && locations.length>4 && locations.length<=50){
    ...
    function gstvalidaction() {
    	...
    	console.log(gstcs)
    	var button=document.getElementById("button");
    	if(gstcs!="" && gstcs.length>4 && gstcs.length<=20){
   ...
   function ownamevalidaction() {
	...
	console.log(ownames)
	var button=document.getElementById("button");
		if(ownames!="" && ownames.length>3 && ownames.length<=25){
			document.getElementById("own").innerHTML="";
			 button.removeAttribute("Disabled");
		}else{
			document.getElementById("own").innerHTML="<span style='color:red;'>must be 3 to 25 characters</span>";
			 button.setAttribute("Disabled","");
	            return;
		}
	}
    ...
    function form(){
        ...
        var button=document.getElementById("button");
        if(names.length!="" && names.length>4 && names.length<=25 
          &&  locations!="" && locations.length>4 && locations.length<=50 &&
          gstcs!="" &&   gstcs.length>4 && gstcs.length<=20 &&
          ownames!="" && ownames.length>4 && ownames.length<=25 &&
