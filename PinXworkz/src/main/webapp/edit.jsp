<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Insert title here</title>
<title>Insert title here</title>
 <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous" >
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
    <style>
    .container {
    margin-left: 90px;
    max-width: 86vw;
    margin-top: 60px;
    padding: 20px;
    background-color: #edf4f511;
   box-shadow: 10px 18px 16px rgb(128, 196, 230);
    border-radius: 8px;
    text-overflow: clip;
    
    

width: 200ch;
             border-right: 2px solid;
             border-bottom: 3px solid;
             border-top:1px solid;
            overflow: hidden;
            animation: type 2.5s steps(100);
              white-space: nowrap;

}

@keyframes type {
        from {
             width: 0;
            
            background-color:rgb(128, 196, 230) ;
            color: #ccc;
            box-shadow: none;
            transform: translateY(-550px);
            
             
        }
    }


.container h2 {
    font-size: 40px;
    /* margin: 10px; */
    
}

/* Form Input Styling */
.container label {
    font-weight: bold;
    display:block;
    margin-top: 30px;

}

.container input {
    width: 45%;
    padding: 13px;
    margin-top: 5px;
    margin-left: 0px;
    border-radius: 4px;
    /* position:relative; */
    box-sizing: border-box;
border: 1px solid #ccc;
}
.container .input1{
    width: 95%;
    padding: 10px;
    margin-top: -120px;
    margin-left: 650px;
    /* border: 1px solid #ccc; */
    border-radius: 4px;
    box-sizing: border-box;
    
}

/* Form Button Styling */
.container button {
    background-color: #007BFF;
    color: #fff;
    border: none;
    border-radius: 4px;
    padding: 10px 20px;
    cursor: pointer;
    margin-top: 10px;
    margin-left:10px;

}

.container button:hover {
    background-color: #0056b3;
}

    
     .footer{
     margin-top: 60px;
     background-color: #88ceec;x
     }

    </style>

</head>
<body>
    <nav class="navbar navbar-expand-lg bg-body-tertiary"  >
        <div class="container-fluid" style="background-color:  #88ceec;">
              
            <div>
                <img src="https://x-workz.in/static/media/Logo.cf195593dc1b3f921369.png" alt="logo" style="height: 80px; width: 160px;">
            </div>
            
             <img alt="person" style="height:80px;width: 80px; border-radius: 50%; margin-left:55vh; margin-top: 5px"
            src='display?fileName=${picPath eq "null" ? "temp.jpg" : picPath}'>
         <b>${vdata.emailid }</b>
          <a class="navbar-brand" href="backtovprofilep"  style="margin-left: 500px ; font-size: 35px; color:white;"> <b>Home</b></a>
          <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
          </button>
          
        </div>
      </nav>
      
       <div class="container">
         <marquee behavior="alternate" direction="right" scrollamount="20" style="background-color:;"> <h2 style="text-shadow: 0px 0px 0px ; color: rgb(16, 17, 17);"> <b>Updateing_Profile</b></h2></marquee>
        

        <form action="update" method="post">

           
            <label for="name">Name:<span id="validname" style="margin-left: 20px"></span></label> <div class="input">
            <input type="text" id="name" name="name" placeholder="enter you are name" value="${vdata.name}" onchange="namevalidation()">
             </div>
             
             <div class="input1">
            <label for="location">Location:<span id="validloc" style="margin-left: 20px"></span></label><span id="ok" ></span>
            <input type="text" id="location" name="location" placeholder="enter you are location" value="${vdata.location}" onchange="locationvalidation()">
            </div>

            <div class="input">
            <label for="gstno">GST NO:<Span id="gstvl" style="margin-left:20px; " ></Span><span id="ajaxgstvalid" style="color:red"></span></label>
            <input type="text" id="gstno" name="gstno"  placeholder="enter gstno" value="${vdata.gstno }" onchange="gstvalidaction()">
            </div>
             
            <div class="input1">
            <label for="companystartdate">Company Start Date:</label><span id="ok" ></span>
            <input type="date" id="companydate" name="companyStartDate" placeholder="enter start date" value="${vdata.companyStartDate}" >
            </div>

            <div class="input">
            <label for="ownername">Owner Name:<Span id="own" style="margin-left:20px; " ></Span></label><span id="ok" ></span>
            <input type="text" id="ownername" name="ownername" placeholder="enter owner name" value="${vdata.ownername }" onchange="ownamevalidaction()">
            </div>


            <div class="input1">
            <label for="servicetype">Service Type:<Span id="stype" style="margin-left:20px; " ></Span></label><span id="ok" ></span>
            <input type="text" id="servicetype" name="servicetype" placeholder="enter you are service" value="${vdata.servicetype }" onchange="stypevalidation()" >
            </div>

            <div class="input">

            <label for="contactno">Contact Num:<Span id="contact" style="margin-left:20px; " ></Span><Span id="ajaxContactValida"  style="color:red; "></Span></label><span id="ok" ></span>
            <input type="number" id="contactno" name="contactno" placeholder="enter you are contact" value="${vdata.contactno }" onchange="contactvalidatioin()" >
            </div>
<!--             <Span id="validcontajax" style="color:red;" ></Span> -->

            <div class="input1">
            <label for="altercontactno">Alternative Num:<Span id="altnumb" style="margin-left:20px; " ></label><span id="ok" ></span>
            <input type="tel" id="altercontactno" name="alternativeno" placeholder="enter you are alternumber" value="${ vdata.alternativeno}" onchange="alternumvalidation()">
            </div>


             <div class="input">
            <label for="email">Email Id:<span id="emailvalid" style="margin-left:20px;"></span><span id="ajaxemilvalid" style="color:red;"></span></label>
            <input type="email" id="emailid" name="emailid" placeholder="enter you are email" value="${vdata.emailid }" onchange="emailvalidation()">
             </div>



               <div class="input1">
                 <label for="wibsite">Web Sit:</label><span id="ok" ></span><Span id="websitevalid" style="color:red;"></Span>
                <input type="url" name="website" id="website" placeholder="enter you are website" value="${vdata.website}" onchange="websitvalidaction()"><br>
                <Span id="validwebsiteajax" style="color:red;"></Span>

                </div>


                  <input type="hidden" name="id" value="${vdata.id}" id="id">
                  <input type="hidden" name="createdby" value="${vdata.createdby}" id="createdby">
                  <input type="hidden" name="createdate" value="${vdata.createdate}" id="createdate">
                  <input type="hidden" name="loginCount" value="${vdata.loginCount}" id="loginCount">
                  <input type="hidden" name="accountLockTime" value="${vdata.accountLockTime}" id="accountLockTime">

            <a href="#"><button id="button" onclick="update()">Update</button><br>    </a>
            
           
            
            
            <p style="margin-left: 660px; margin-top: -30px;">Don't have Account?  <a href="">Rigester</a></p>
        </form>
    </div>

      
      
      
      <div class="footer" >
      <div>
        <img src="https://x-workz.in/static/media/Logo.cf195593dc1b3f921369.png" alt="logo" style="height: 80px; width: 160px; margin-left: 570px;">
        <p style="margin-left: 620px;"><b> follow us on</b></p> 
        
        <img style="margin-left: 800px; margin-top: -160px" alt="" src="data:image/png;base64," style="margin-left: 250px;">
       
        <img  style="margin-top: -160px; margin-left:10px; border-radius:50px; width:35px;" alt="twiterlogo" src="data:image/png;base64,">
        
        <img style="margin-top: -160px; margin-left:10px;" alt="instalogo" src="data:image/png;base64,">
        <img style="margin-top: -160px; margin-left:10px" alt="linkedinlogo" src="data:image/png;base64,">
        <p style="margin-left: 650px; margin-top: -20px">Copyright © 2022, All Right Reserved</p>
        </div>


      <script>
      
      function namevalidation(){
          let names=document.getElementById("name").value;
          console.log(names)
          if(names.length>4 && names.length<=25){
              document.getElementById("validname").innerHTML="";
          }else{
              document.getElementById("validname").innerHTML="<span style='color:red;'>must be 4 & 25</span>";
          }

      }
      

      function locationvalidation() {
      	let locations=document.getElementById("location").value;
      	console.log(locations)
      	if(locations.length>4 && locations.length<=25){
      		document.getElementById("validloc").innerHTML="";
      	}else{
      		document.getElementById("validloc").innerHTML="<span style='color:red;'>must be 4 & 25</span>"
      	}
  		
  	}
      
      function gstvalidaction() {
      	let gstcs=document.getElementById("gstno").value;
      	console.log(gstcs)
      	if(gstcs.length>4 && gstcs.length<=10){
      		document.getElementById("gstvl").innerHTML="";
      		 const xhttp= new XMLHttpRequest();
               xhttp.open("GET","http://localhost:8080/PinXworkz/uniquegstno/"+gstcs);
               xhttp.send();

               xhttp.onload=function(){
               console.log(this);
               document.getElementById("ajaxgstvalid").innerHTML=this.responseText;
               }
      	}else{
      		document.getElementById("gstvl").innerHTML="<span style='color:red;'>must be 4 & 10</span>";
      	}
      }
      	

  	
      function ownamevalidaction() {
   	let ownames=document.getElementById("ownername").value;
   	console.log(ownames)
   		if(ownames.length>4 && ownames.length<=25){
   			document.getElementById("own").innerHTML="";
   		}else{
   			document.getElementById("own").innerHTML="<span style='color:red;'>must be 4 & 25</span>";
   		}
   	}
       
        function stypevalidation() {
       	let stypes=document.getElementById("servicetype").value;
       	console.log(stypes)
       	if(stypes.length>4 && stypes.length<=25){
       		document.getElementById("stype").innerHTML="";
       	}else{
       		document.getElementById("stype").innerHTML="<span style='color:red;'>must be 4 & 25</span>";
       	} 	
   	}
        
        function contactvalidatioin() {
       	 let numbers= document.getElementById("contactno").value;
       	 console.log(numbers)
       	 if(numbers!="" && numbers.length=="10"){
       		 document.getElementById("contact").innerHTML="";
       		 const xhttp= new XMLHttpRequest();
                xhttp.open("GET","http://localhost:8080/PinXworkz/uniqueContact/"+numbers);
                xhttp.send();

                xhttp.onload=function(){
                console.log(this);
                document.getElementById("ajaxContactValida").innerHTML=this.responseText;
                }
       	 }else{
       		 document.getElementById("contact").innerHTML="<span style='color:red;'>must be 10</span>";
       	 }
       	 	
   	}
        
        function alternumvalidation() {
       	 let alternumber= document.getElementById("altercontactno").value;
       	 console.log(alternumber)
       	 if(alternumber!="" && alternumber.length=="10"){
       		 document.getElementById("altnumb").innerHTML="";
       	 }else{
       		document.getElementById("altnumb").innerHTML="<span style='color:red;'>must be 10</span>";
       	 }
       	 
   	}
       
       function emailvalidation() {
       	let emails=document.getElementById("emailid").value;
       	console.log(emails)
       	if(emails.length>5 && emails.length<=45){
       		document.getElementById("emailvalid").innerHTML="";
       		
       		 const xhttp= new XMLHttpRequest();
                xhttp.open("GET","http://localhost:8080/PinXworkz/uniqueEmail/"+emails);
                xhttp.send();

                xhttp.onload=function(){
                console.log(this);
                document.getElementById("ajaxemilvalid").innerHTML=this.responseText;
                }
       	}else{
       		document.getElementById("emailvalid").innerHTML="<span style='color:red;'>must be valid</span>";
       	}	
   	}
       

      
      
      </script>

</body>
</html>